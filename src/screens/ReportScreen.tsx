  ...
  return (
    <ScrollView style={styles.root} showsVerticalScrollIndicator={false}>
      ...
      {FATIGUE_LEVELS.map(level => {
        ...
        return (
          <View key={level.state} style={[styles.classRow, isCurrent && { backgroundColor: level.bgColor, borderLeftColor: level.color, borderLeftWidth: 4 }]}>
            <View style={[styles.dot, { backgroundColor: level.color }]} />
            <View style={{ flex: 1 }}>
              <Text style={[styles.classLabel, isCurrent && { fontWeight: '700', color: colors.tp }]}>{level.label}</Text>
      ...
      {/* Band power bars */}
      <Text style={[styles.sectionTitle, { marginTop: 20 }]}>Absolute Band Powers (V²/Hz)</Text>
      <Text style={styles.sectionSub}>From psd_welch() · n_fft=256 · fmin=1 · fmax=50</Text>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 13, fontWeight: '600', color: colors.tp, marginTop: 16 },
  sectionSub: { fontSize: 9, color: colors.tl, marginTop: 3, marginBottom: 10 },

  classRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.bg2, borderRadius: 10, padding: 12, marginBottom: 8, gap: 8 },
  dot: { width: 10, height: 10, borderRadius: 5 },
  classLabel: { fontSize: 12, color: colors.ts },
  classDesc: { fontSize: 9, color: colors.tl, marginTop: 2 },
  classTBR: { fontSize: 10, fontWeight: '600' },
  youBadge: { fontSize: 8, fontWeight: '700', color: colors.warn, marginLeft: 4 },
  ...
  compareCard: { backgroundColor: colors.bg2, borderRadius: 12, padding: 14, marginTop: 16 },
  compareLabel: { fontSize: 11, fontWeight: '600', color: colors.ts },
  compareValue: { fontSize: 20, fontWeight: '700', color: colors.warn, marginTop: 6 },
  compareSub: { fontSize: 9, color: colors.tl, marginTop: 6 },
});
