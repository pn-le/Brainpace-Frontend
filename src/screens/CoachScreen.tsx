...
const PHASES = [
  { label: 'Inhale', color: colors.okGreen },
  { label: 'Hold',   color: colors.violetle },
      ...
      <View style={{ alignItems: 'center' }}>
        ...
        <View style={{ width: svgSize, height: svgSize }}>
          <Svg width={svgSize} height={svgSize}>
            {/* ghost square */}
            <Path d={sqPath} stroke={colors.inkBorder} strokeWidth={2} fill="none" />
            {/* animated stroke */}
            <AnimatedPath
              d={sqPath}
...
export default function CoachScreen() {
  ...
  const stateColor =
    STATE === 'CALM' ? colors.okGreen :
    STATE === 'ELEVATED' ? colors.amber :
    colors.coral;
        ...
        return (
          ...
          >
            ...
            <View style={styles.exMid}>
              <Text style={[styles.exName, active && { color: colors.textPrimary }]}>{ex.name}</Text>
...
const styles = StyleSheet.create({
  ...
  // Section 1
  card: { backgroundColor: colors.inkCard, borderRadius: 16, padding: 16, marginBottom: 14 },
  sectionTag: { fontSize: 10, fontWeight: '700', color: colors.textSecondary, letterSpacing: 1, marginBottom: 12 },
  statePill: {
  ...
  barTrack: {
    height: 8, backgroundColor: colors.inkBorder, borderRadius: 4, overflow: 'hidden',
  },
  barFill: {
    height: 8, borderRadius: 4, backgroundColor: colors.violet,
    shadowColor: colors.violet, shadowOpacity: 0.7, shadowRadius: 8,
    shadowOffset: { width: 0, height: 0 },
  },
  calmPctText: { fontSize: 11, color: colors.textSecondary, marginTop: 8 },

  // Section 2
  coachHeading: { fontSize: 16, fontWeight: '700', color: colors.violet, marginBottom: 6 },
  coachReason: { fontSize: 13, color: colors.textMuted, fontStyle: 'italic', marginBottom: 4 },
  coachExercise: { fontSize: 18, fontWeight: '700', color: colors.textPrimary, marginBottom: 16 },
  ctaBtn: {
    backgroundColor: colors.violet, borderRadius: 12, paddingVertical: 14,
    alignItems: 'center',
    shadowColor: colors.violet, shadowOpacity: 0.5, shadowRadius: 12,
    shadowOffset: { width: 0, height: 0 },
  },
  ctaBtnText: { fontSize: 15, fontWeight: '700', color: colors.white },

  // Section 3
  listHeader: { fontSize: 13, fontWeight: '600', color: colors.textSecondary, letterSpacing: 0.5, marginBottom: 10 },
  exRow: {
    flexDirection: 'row', alignItems: 'center',
    backgroundColor: colors.inkCard, borderRadius: 14,
  ...
  exAccent: {
    ...
    width: 3, backgroundColor: colors.violet,
  },
  exIcon: { fontSize: 22 },
  exMid: { flex: 1, gap: 3 },
  exName: { fontSize: 14, fontWeight: '600', color: colors.textSecondary },
  exTag: { fontSize: 10, color: colors.textMuted },
  exDur: { fontSize: 12, color: colors.textSecondary },

  // Section 4
  playerCard: {
    backgroundColor: colors.inkCard, borderRadius: 20, padding: 20,
    marginTop: 6, marginBottom: 14, alignItems: 'center', gap: 20,
  },
  phaseLabel: { fontSize: 22, fontWeight: '700' },
  phaseCountdown: { fontSize: 42, fontWeight: '800', color: colors.textPrimary, lineHeight: 48 },
  stopBtn: {
    width: '100%', backgroundColor: colors.inkBorder, borderRadius: 12,
    paddingVertical: 13, alignItems: 'center',
  },
  stopBtnText: { fontSize: 14, fontWeight: '600', color: colors.textSecondary },
});
